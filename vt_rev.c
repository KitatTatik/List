...
// TODO: implement this function
    ...
    reverse_list_recursive(&zanav);  // repeat with the tail, that is now shorter by 1
    rosh->next->next = rosh;   // set first to the end of list
    
#ifdef DEBUG
    printf("DEBUG AFTER rosh->next = %llu\n", (long long unsigned int) rosh->next);
#endif
    rosh->next = NULL;         // zeroing of the first
    *head = zanav;             // fixing the head
}
